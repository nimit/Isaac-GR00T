...
from gr00t.model.backbone.eagle_backbone import (
...
def get_input_info(policy, observations):
    ...
    return normalized_input["attention_mask"], normalized_input["state"]
...
def export_eagle2_llm(backbone_model, backbone_config, output_dir, attention_mask):
    class EagleBackboneOpt(EagleBackbone):
        ...
        def forward(self, input_ids, vit_embeds, attention_mask):
            ...
            vit_embeds = vit_embeds.reshape(vit_embeds.shape[0], h, w, -1)  # [B, 16, 16, 1152]
    ...
    input_ids[:, : model.model.num_image_token] = model.model.img_context_token_id
...
def export_action_head(policy, ONNX_export_path, input_state, attention_mask):
    ...
    vl_embs_tensor = torch.randn(
        (1, attention_mask.shape[1], policy.model.action_head.config.input_embedding_dim),
        dtype=torch.float16,
    ).cuda()
...
def run_groot_inference(
    ...
    export_eagle2_vit(policy.model.backbone.model.vision_model.vision_model, onnx_model_path)
    export_eagle2_llm(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="Isaac-GR00T/GR00T-N1-2B",
    )
