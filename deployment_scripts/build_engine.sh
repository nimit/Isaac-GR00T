...
echo "2: The The MIN_LEN/OPT_LEN/MAX_LEN for LLM, DiT models is set to 80/99/120."
echo "If your inference batch size exceeds 8 or input embeds length for LLM and DiT not fit your use case, please set it to your actual batch size and length variables."
...
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers --onnx=gr00t_onnx/action_head/DiT.onnx --saveEngine=gr00t_engine/DiT.engine --minShapes=sa_embs:1x17x1536,vl_embs:1x${MIN_LEN}x1536,timesteps_tensor:1  --optShapes=sa_embs:1x17x1536,vl_embs:1x${OPT_LEN}x1536,timesteps_tensor:1  --maxShapes=sa_embs:8x17x1536,vl_embs:8x${MAX_LEN}x1536,timesteps_tensor:8 > gr00t_engine/build_DiT.log 2>&1
